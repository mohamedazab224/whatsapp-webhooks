# دليل إعداد Azure OpenAI Agent

## نظرة عامة

تم إضافة Azure OpenAI Agent للرد التلقائي الذكي على رسائل العملاء وإدارة طلبات الصيانة باستخدام الذكاء الاصطناعي.

---

## المعرفات المطلوبة

```env
AZURE_OPENAI_KEY=B9miWBdymm8e4BUFoxDMqs4UB7luinaBG7c22i5uNwOGn5zNaNE3JQQJ99BKACYeBjFXJ3w3AAABACOG8ZhR
AZURE_OPENAI_DEPLOYMENT=AzaBot
AZURE_OPENAI_ENDPOINT=https://azabotai.openai.azure.com
AZURE_OPENAI_KEY2=B4CcYQDenUwAd4FTSJg0TRIL4yDYP2qRoHS0EHsF8Wwe4lkfJN0sJQQJ99BKACYeBjFXJ3w3AAABACOGlr31
```

---

## المميزات

### 1. الرد التلقائي الذكي
- رد فوري على رسائل العملاء
- فهم اللغة العربية والإنجليزية
- سياق المحادثة محفوظ لكل عميل

### 2. إنشاء طلبات الصيانة
يقوم AI Agent تلقائياً بإنشاء طلب صيانة عند:
- استخدام العميل لكلمات مثل: "طلب صيانة"، "محتاج فني"، "حجز موعد"
- تحديد نوع المشكلة (سباكة، كهرباء، تكييف، إلخ)
- تصنيف الطوارئ (عادي، مهم، عاجل)

### 3. أنواع الخدمات المدعومة

| الفئة | الوصف | أمثلة |
|------|-------|-------|
| السباكة | تسريب مياه، انسداد مجاري، خزانات | "عندي تسريب ماء" |
| الكهرباء | مشاكل كهربائية، إضاءة، تمديدات | "الكهرباء مقطوعة" |
| التكييف | صيانة مكيفات، تعبئة فريون، تنظيف | "المكيف ما يبرد" |
| النجارة | صيانة أبواب، نوافذ، أثاث | "الباب مكسور" |
| الدهان | دهانات، تشطيبات | "ابغى دهان" |
| أخرى | أي خدمات صيانة أخرى | - |

---

## كيف يعمل

### 1. استقبال الرسالة
```
عميل → WhatsApp → Webhook → AI Agent
```

### 2. معالجة الرسالة
```javascript
// AI Agent يقوم بـ:
1. فهم سياق المحادثة
2. تحديد نوع الطلب
3. استخراج المعلومات (نوع الخدمة، الطوارئ)
4. توليد رد مناسب
5. إنشاء طلب صيانة إذا لزم
```

### 3. إرسال الرد
```
AI Agent → WhatsApp API → عميل
```

---

## أمثلة على المحادثات

### مثال 1: طلب صيانة سباكة

**العميل:**
```
السلام عليكم، عندي تسريب ماء في المطبخ ضروري
```

**AI Agent:**
```
وعليكم السلام ورحمة الله وبركاته 👋

تم استلام طلبكم بخصوص تسريب المياه في المطبخ.

📋 تفاصيل الطلب:
- النوع: سباكة
- المستوى: عاجل
- رقم الطلب: REQ-1234567890

سيتم التواصل معكم من قبل الفني المختص خلال 30 دقيقة.

هل هناك تفاصيل إضافية تود إضافتها؟
```

### مثال 2: استفسار عام

**العميل:**
```
كم سعر صيانة المكيف؟
```

**AI Agent:**
```
أهلاً بك! 😊

أسعار صيانة المكيفات:
• تنظيف عادي: 150 ريال
• تنظيف شامل: 250 ريال
• تعبئة فريون: 200 ريال
• صيانة كاملة: 400 ريال

هل تريد حجز موعد لصيانة المكيف؟
```

### مثال 3: تحويل للموظف

**العميل:**
```
ابغى اتكلم مع موظف
```

**AI Agent:**
```
حاضر، سأحولك لأحد موظفي خدمة العملاء الآن... ⏳

→ [يتم التحويل لموظف بشري]
```

---

## التكامل مع Webhook Router

AI Agent متكامل مع نظام Webhook Router:

```javascript
// في webhook/route.ts
if (routing.action === "bot" && message.text?.body) {
  // استدعاء AI Agent
  const aiResponse = await azureOpenAIAgent.respondToMessage(
    message.from,
    message.text.body
  )
  
  // إرسال الرد
  await sendWhatsAppMessage({
    phoneNumber: message.from,
    message: aiResponse
  })
}
```

---

## واجهة إدارة AI Agent

### الوصول
```
http://yourdomain.com/control → تاب "AI Agent"
```

### المميزات المتاحة:

1. **الإحصائيات**
   - إجمالي المحادثات
   - عدد طلبات الصيانة
   - الطلبات قيد الانتظار
   - الطلبات العاجلة

2. **إدارة الطلبات**
   - عرض جميع طلبات الصيانة
   - تحديث حالة الطلب
   - فلترة حسب الفئة والأولوية

3. **الإجراءات المتاحة**
   - تغيير الحالة: قيد التنفيذ
   - تغيير الحالة: مكتمل
   - إلغاء الطلب

---

## API Endpoints

### 1. الحصول على طلبات الصيانة
```bash
GET /api/ai-maintenance
```

**Response:**
```json
{
  "success": true,
  "requests": [
    {
      "id": "REQ-1234567890",
      "customerPhone": "966xxxxxxxxx",
      "issueDescription": "تسريب ماء في المطبخ",
      "urgency": "urgent",
      "category": "plumbing",
      "status": "pending",
      "createdAt": "2024-01-01T12:00:00.000Z"
    }
  ]
}
```

### 2. تحديث حالة طلب
```bash
POST /api/ai-maintenance
Content-Type: application/json

{
  "requestId": "REQ-1234567890",
  "updates": {
    "status": "in_progress",
    "assignedTo": "فني محمد"
  }
}
```

### 3. إحصائيات AI Agent
```bash
GET /api/ai-stats
```

**Response:**
```json
{
  "success": true,
  "stats": {
    "totalConversations": 45,
    "totalMaintenanceRequests": 12,
    "pendingRequests": 3,
    "urgentRequests": 1
  }
}
```

---

## التخصيص

### تعديل System Prompt

في ملف `lib/azure-openai-agent.ts`:

```typescript
private getSystemPrompt(): string {
  return `أنت مساعد ذكي لشركة Uberfix...
  
  // أضف التخصيصات هنا:
  - معلومات إضافية عن الشركة
  - أسعار محددة
  - سياسات خاصة
  - لغة الرد والأسلوب
  `
}
```

### إضافة فئة خدمة جديدة

```typescript
// في extractCategory()
if (lowerText.includes("تنظيف") || lowerText.includes("نظافة")) {
  return "cleaning"
}
```

### تخصيص Trigger Keywords

```typescript
// في analyzeForMaintenanceRequest()
const requestKeywords = [
  "طلب صيانة",
  "محتاج فني",
  // أضف كلمات جديدة هنا
  "عايز خدمة",
  "ممكن تجي",
]
```

---

## الأمان والخصوصية

### حفظ البيانات
- المحادثات محفوظة في الذاكرة (RAM)
- آخر 20 رسالة فقط لكل عميل
- يتم مسح البيانات عند إعادة تشغيل السيرفر

### حماية المعلومات
- API Keys مخزنة في متغيرات البيئة
- لا يتم تسجيل بيانات حساسة
- جميع الاتصالات عبر HTTPS

---

## استكشاف الأخطاء

### خطأ: "Azure OpenAI API key not configured"

**الحل:**
1. تأكد من وجود `AZURE_OPENAI_KEY` في `.env.local`
2. تحقق من صحة المفتاح
3. أعد تشغيل السيرفر

### AI لا يرد

**تحقق من:**
1. Webhook Router يوجه للـ Bot: `routing.action === "bot"`
2. الرسالة نصية: `message.type === "text"`
3. Console logs: `[v0] AI response sent successfully`

### الردود بطيئة

**الحلول:**
- استخدم `max_tokens` أقل (حالياً 800)
- قلل `temperature` للردود الأسرع
- استخدم deployment أسرع

---

## التطوير المستقبلي

### ميزات مخططة:

1. **تكامل مع قاعدة بيانات**
   - حفظ المحادثات بشكل دائم
   - تاريخ كامل لكل عميل

2. **تحليلات متقدمة**
   - تقارير الأداء
   - معدل الرضا
   - أوقات الذروة

3. **Multilingual Support**
   - دعم لغات إضافية
   - ترجمة تلقائية

4. **Rich Media Responses**
   - إرسال صور وفيديوهات
   - قوائم تفاعلية
   - أزرار ديناميكية

---

## الدعم الفني

للمساعدة أو الأسئلة:
- راجع: `PRODUCTION_SETUP.md`
- Documentation: `README.md`
- Azure OpenAI Docs: [docs.microsoft.com](https://learn.microsoft.com/en-us/azure/ai-services/openai/)

---

✅ **Azure OpenAI Agent جاهز للعمل!**
